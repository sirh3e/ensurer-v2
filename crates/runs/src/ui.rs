...
fn current_mode(app: &App) -> (&'static str, Color, Color) {
    match &app.overlay {
        Overlay::Command(buf) if buf.starts_with('/') =>
            ("SEARCH",  palette::BASE, palette::GREEN),
        Overlay::Command(_) =>
            ("COMMAND", palette::BASE, palette::YELLOW),
        Overlay::Filter =>
            ("FILTER",  palette::BASE, palette::MAUVE),
        Overlay::Confirm(_) =>
            ("CONFIRM", palette::BASE, palette::RED),
        Overlay::Help =>
            ("HELP",    palette::BASE, palette::TEAL),
        Overlay::None => {
