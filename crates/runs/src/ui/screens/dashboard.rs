...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    let main_area = Rect { height: area.height.saturating_sub(1), ..area };
    ...
    let outer = Block::default()
        ...
        .title(Span::styled(" Dashboard ", Style::default().fg(palette::LAVENDER).add_modifier(Modifier::BOLD)));
...
fn render_run_stats(f: &mut Frame, area: Rect, app: &App) {
    let counts = |status: RunStatus| {
        app.runs.iter().filter(|r| r.status == status).count()
    };
    let total = app.runs.len();

    let rows = vec![
        stat_line("pending",        counts(RunStatus::Pending),            run_status_color(RunStatus::Pending)),
        stat_line("running",        counts(RunStatus::Running),            run_status_color(RunStatus::Running)),
        stat_line("succeeded",      counts(RunStatus::Succeeded),          run_status_color(RunStatus::Succeeded)),
        stat_line("failed",         counts(RunStatus::Failed),             run_status_color(RunStatus::Failed)),
        stat_line("cancelled",      counts(RunStatus::Cancelled),          run_status_color(RunStatus::Cancelled)),
        stat_line("partial",        counts(RunStatus::PartiallySucceeded), run_status_color(RunStatus::PartiallySucceeded)),
        Line::from(Span::styled("─".repeat(28), Style::default().fg(palette::SURFACE1))),
...
fn render_calc_stats(f: &mut Frame, area: Rect, app: &App) {
    let all_calcs: Vec<CalcStatus> = app.runs.iter()
    ...
    let rows = vec![
        stat_line("pending",   counts(CalcStatus::Pending),   calc_status_color(CalcStatus::Pending)),
        stat_line("running",   counts(CalcStatus::Running),   calc_status_color(CalcStatus::Running)),
        stat_line("retrying",  counts(CalcStatus::Retrying),  calc_status_color(CalcStatus::Retrying)),
        stat_line("succeeded", counts(CalcStatus::Succeeded), calc_status_color(CalcStatus::Succeeded)),
        stat_line("failed",    counts(CalcStatus::Failed),    calc_status_color(CalcStatus::Failed)),
        stat_line("cancelled", counts(CalcStatus::Cancelled), calc_status_color(CalcStatus::Cancelled)),
        Line::from(Span::styled("─".repeat(28), Style::default().fg(palette::SURFACE1))),
        stat_line("total", total, palette::TEXT),
    ];
    ...
    let para = Paragraph::new(rows)
        ...
        .block(
            Block::default()
                ...
                .border_style(Style::default().fg(palette::SURFACE1))
                .style(Style::default().bg(palette::BASE))
                .title(Span::styled(" Calculations ", Style::default().fg(palette::MAUVE))),
        );
    f.render_widget(para, area);
}
...
fn render_jobs(f: &mut Frame, area: Rect, app: &App) {
    ...
    let active: Vec<&Run> = app.runs.iter()
        .filter(|r| r.status == RunStatus::Running || r.status == RunStatus::Pending)
        .collect();

    let history: Vec<&Run> = {
        let mut v: Vec<&Run> = app.runs.iter()
...
fn active_job_line(run: &Run, now: chrono::DateTime<Utc>) -> Line<'static> {
    let done = run.calculations.iter().filter(|c| c.status.is_terminal()).count();
...
fn history_line(run: &Run, now: chrono::DateTime<Utc>) -> Line<'static> {
    let done = run.calculations.iter().filter(|c| c.status == CalcStatus::Succeeded).count();
    let total = run.calculations.len();
